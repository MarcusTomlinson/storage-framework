...
public:
    ...
    Handler(QObject* parent,
            ...
            std::function<void(decltype(reply) const&, QFutureInterface<T>&)> const& closure);
...
template<typename T>
template<typename ... DBusArgs>
Handler<T>::Handler(QObject* parent, QDBusPendingReply<DBusArgs...> const& reply, std::function<void(decltype(reply) const&, QFutureInterface<T>&)> const& closure)
    : HandlerBase(parent,
                  reply,
